...
class Message {
	...
	public function getLatestMessage($userLoggedIn, $user2) {
		...
		$row = mysqli_fetch_array($query);
		$sent_by = ($row['user_to'] == $userLoggedIn) ? "Gelen: " : "Cevap: ";
		...
		if($interval->y >= 1) {
			if($interval->y == 1)
				$time_message = $interval->y . " year önce"; //1 year önce
			else
				$time_message = $interval->y . " years önce"; //1+ year önce
		}
			...
			if($interval->s < 30) {
				...
				$time_message = $interval->s . " dakika önce";
			}
		}
	...
	public function getConvosDropdown($data, $limit) {
		...
		if($count > $limit)
			...
			$return_string .= "<input type='hidden' class='noMoreDropdownData' value='true'> <p style='text-align: center;'>Yüklenecek daha fazla mesaj yok!</p>";
